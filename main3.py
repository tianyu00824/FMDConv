...
def main():
    ...
    if args.evaluate:
        print('\nEvaluation only')
        with torch.no_grad():
            test_loss, test_acc = test(val_loader, val_loader_len, model, criterion)
...
def test(val_loader, val_loader_len, model, criterion):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        if (i % args.print_freq == 0) or i == val_loader_len-1:
            print('({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Loss: {loss:.4f} | top1: {top1: .4f} | '
                   'top5: {top5: .4f}'.format(
                        batch=i + 1,
                        size=val_loader_len,
...
def validate(val_loader, val_loader_len, model, criterion, use_cuda):
    global best_acc
